...
class EmailParser:
    """Parser to clean and extract the latest message from email body"""
    ...
    @classmethod
    def clean_body(cls, body: str, message_id: Optional[str] = None) -> str:
        """
        ...
        if not body:
            ...
            logger.info(f"Processed email {message_id or 'unknown'} in {end_time - start_time:.4f} seconds")
            return ""
        
        # First convert any HTML to plain text
        cleaned_html = cls._convert_html_to_text(body)
            
        # Find the first occurrence of any divider pattern in the cleaned HTML
        first_divider_pos = cls._find_first_divider(cleaned_html)
    ...
    @classmethod
    def _convert_html_to_text(cls, text: str) -> str:
        """
        ...
        try:
            ...
            try:
                ...
                for i, table in enumerate(tables):
                    try:
                        ascii_table = cls._convert_table_to_ascii(table)
    ...
    @classmethod
    def _convert_table_to_ascii(cls, table) -> str:
        """
    ...
    @classmethod
    def _find_first_divider(cls, text: str, lazy=False) -> Optional[int]:
        ...
        for cre in cls.COMPILED_DIVIDER_PATTERNS:
        ...
        if len(text) > max_search_length:
            for cre in cls.COMPILED_DIVIDER_PATTERNS[:3]:
